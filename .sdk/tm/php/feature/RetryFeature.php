...
// WorldWonders SDK retry feature
...
class WorldWondersRetryFeature extends WorldWondersBaseFeature
{
    ...
    public function init(WorldWondersContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (WorldWondersContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(WorldWondersContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(WorldWondersContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
