...
module WorldWondersUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = WorldWondersContext.new({}, nil)
    end
    op = ctx.op || WorldWondersOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || WorldWondersResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(WorldWondersError) ? err.msg : err.to_s
