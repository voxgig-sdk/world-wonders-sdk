...
from projectname_sdk import WorldWondersSDK
from projectname_sdk.core.spec import WorldWondersSpec
from projectname_sdk.core.result import WorldWondersResult
from projectname_sdk.core.response import WorldWondersResponse
from projectname_sdk.core.operation import WorldWondersOperation
from projectname_sdk.core.error import WorldWondersError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import WorldWondersBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return WorldWondersError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = WorldWondersSDK.test(None, None)
        ...
        class TestInitFeature(WorldWondersBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(WorldWondersBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = WorldWondersSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = WorldWondersSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = WorldWondersSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = WorldWondersOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = WorldWondersSDK.test(None, setup_opts)
