...
//     (\`new WorldWondersSDK(...)\` / \`WorldWondersSDK.test(...)\` become
//     \`WorldWondersSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'WorldWondersSDK'
...
const TEST_SEED = {"entity":{"wonder":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new WorldWondersSDK(...)\` and any
// \`WorldWondersSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WorldWondersSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WorldWondersSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
