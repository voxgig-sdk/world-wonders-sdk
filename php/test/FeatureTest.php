...
class FtCtrl extends WorldWondersControl
{
...
class FtClient
{
    ...
    private WorldWondersUtility $utility;

    public function __construct(WorldWondersUtility $utility, array $options)
    {
    ...
    public function get_utility(): WorldWondersUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WorldWondersContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WorldWondersContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new WorldWondersUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (WorldWondersContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WorldWondersConfig::make_feature($name);
    ...
    private static function buildUrl(WorldWondersSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WorldWondersError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WorldWondersResponse($fetched);
        }
    ...
    private function fail(WorldWondersContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WorldWondersContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
