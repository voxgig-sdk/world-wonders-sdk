...
class WorldWondersTestFeature extends WorldWondersBaseFeature
{
    ...
    public function init(WorldWondersContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (WorldWondersContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = WorldWondersParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (WorldWondersContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(WorldWondersContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = WorldWondersParam::call($ctx, $k);
