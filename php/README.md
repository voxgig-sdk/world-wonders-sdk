...
try {
    ...
    foreach ($wonders as $record) {
        $item = $record->data_get();
        echo $item["id"] . " " . $item["build_year"] . "\n";
    }
} catch (\Throwable $err) {
...
print_r(array_map(fn($item) => $item->data_get(), $wonder));
```
...
```
php/
├── worldwonders_sdk.php          -- Main SDK class
...
The main class (`worldwonders_sdk.php`) exports the SDK class
