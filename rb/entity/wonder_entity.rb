# WorldWonders SDK Wonder entity
...
class WonderEntity
  ...
  def make
    opts = @_entopts.dup
    WonderEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = WorldWondersHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Wonder, Hash] the current Wonder data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = WorldWondersHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Wonder fields)
  def match_get
  ...
  #
  # @param reqmatch [WonderLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Wonder.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Wonder, Hash] the loaded Wonder; raises WorldWondersError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = WorldWondersHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [WonderListMatch, Hash, nil] match filter (any subset of
  #   Wonder fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Wonder>, Array] the matching Wonder items; raises WorldWondersError on failure
