...
module WorldWondersUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WorldWondersResponse.new({})
    ...
    if fetch_err
      ...
      response = WorldWondersResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WorldWondersResponse.new(fetched)
    else
