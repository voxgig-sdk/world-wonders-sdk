# WorldWonders Ruby SDK



The Ruby SDK for the WorldWonders API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Wonder` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/world-wonders-sdk/releases](https://github.com/voxgig-sdk/world-wonders-sdk/releases)
...
```ruby
client = WorldWondersSDK.test(testopts, sdkopts)
```
...
raise a `WorldWondersError` (a `StandardError` subclass), so wrap
...
```
rb/
├── WorldWonders_sdk.rb       -- Main SDK module
...
The main module (`WorldWonders_sdk`) exports the SDK class
