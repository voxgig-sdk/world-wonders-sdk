...
require_relative 'WorldWonders_types'
...
class WorldWondersSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = WorldWondersHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = WorldWondersHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, WorldWondersFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    WorldWondersUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = WorldWondersHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = WorldWondersHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = WorldWondersHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = WorldWondersHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = WorldWondersError.new(
        "graphql_error", "WorldWondersSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = WorldWondersSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
