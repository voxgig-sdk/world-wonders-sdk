...
describe("WonderDirect", function()
  it("should direct-list-wonder", function()
    local setup = wonder_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-wonder", setup.live and "live" or "unit")
  ...
  it("should direct-load-wonder", function()
    local setup = wonder_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-wonder", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function wonder_direct_setup(mockres)
