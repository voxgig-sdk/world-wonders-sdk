...
local WonderEntity = {}
WonderEntity.__index = WonderEntity


function WonderEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WonderEntity)
  self._name = "wonder"
  self._client = client
...
function WonderEntity:get_name()
  return self._name
end


function WonderEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WonderEntity.new(self._client, opts)
end
...
function WonderEntity:mark_deleted()
  self._deleted = true
end


function WonderEntity:deleted()
  return true == self._deleted
end


function WonderEntity:data_set(args)
  if args ~= nil then
...
function WonderEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WonderEntity:match_set(args)
  if args ~= nil then
...
function WonderEntity:match_get()
...
function WonderEntity:stream(action, args, callopts)
...
function WonderEntity:_run_op(ctx, post_done)
  local utility = self._utility
