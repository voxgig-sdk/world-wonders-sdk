...
import (
	...
	omni "github.com/voxgig-sdk/world-wonders-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WorldWondersError); is {
				out := map[string]any{
					"name":    "WorldWondersError",
...
func omniCtx(arg any, client *sdk.WorldWondersSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WorldWondersSDK); is {
			client = live
