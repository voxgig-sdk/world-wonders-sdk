...
func TestWonderEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Wonder(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Wonder(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set WORLD_WONDERS_TEST_WONDER_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		wonderRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.wonder")))
		...
		wonderRef01DataDt0LoadResult := core.ToMapAny(entityData(wonderRef01DataDt0Loaded))
...
func wonderBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("WORLD_WONDERS_TEST_WONDER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["WORLD_WONDERS_TEST_WONDER_ENTID"])
