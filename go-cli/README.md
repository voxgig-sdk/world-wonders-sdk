...
boru-driven command-line client **and** interactive REPL for the WorldWonders
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/world-wonders-cli)
make build

# 2. See usage (words, entities, env vars)
./world-wonders-cli --help

# 3. Provide credentials once, via the environment
export WORLD_WONDERS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./world-wonders-cli list wonder
./world-wonders-cli load 1 wonder            # {id:1} shorthand
./world-wonders-cli load '{id:1}' wonder       # explicit match map

# 5. Override the API base URL for a single call
WORLD_WONDERS_BASE=https://api.example.com ./world-wonders-cli list wonder

# 6. No arguments -> interactive REPL
./world-wonders-cli
world-wonders> list wonder
world-wonders> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/world-wonders-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/world-wonders-cli list wonder
   ```
...
```sh
./world-wonders-cli load 1 wonder          # scalar shorthand for {id:1}
./world-wonders-cli load '{id:1}' wonder     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `world-wonders>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `wonder`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/world-wonders-cli`. |
...
(`./world-wonders-cli <expr>`) and interactively (the REPL), and expressions compose the
