...
[MCP](https://modelcontextprotocol.io) server exposing the WorldWonders SDK as
two agent tools — `world-wonders_list` and `world-wonders_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/world-wonders-mcp)
make build

# 2. Provide credentials via the environment
export WORLD_WONDERS_APIKEY=sk_live_xxx
...
```jsonc
// world-wonders_list: first page of records
{ "entity": "wonder" }
{ "entity": "wonder", "query": { } }

// world-wonders_load: one record by id
{ "entity": "wonder", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/world-wonders-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user world-wonders \
     -- "$PWD"/dist/*/world-wonders-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `world-wonders_list` and `world-wonders_load` tools now appear
   in new sessions. Ask the agent to *"list wonder using world-wonders"*
   and it calls `world-wonders_list` with `{"entity":"wonder"}`.
...
```sh
export WORLD_WONDERS_APIKEY=sk_live_xxx            # API key
export WORLD_WONDERS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `world-wonders_list` tool
...
### Call the `world-wonders_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `world-wonders_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `world-wonders_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WORLD_WONDERS_APIKEY` | API key sent with every request. |
| `WORLD_WONDERS_BASE` | Optional override of the API base URL. |
