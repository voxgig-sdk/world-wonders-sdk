...
class WorldWondersTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WorldWondersTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WorldWondersTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WorldWondersTestRunner.getenv("WORLD_WONDERS_TEST_LIVE")
        override = WorldWondersTestRunner.getenv("WORLD_WONDERS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WorldWondersTestRunner.getenv(key)
        ...
        explain = WorldWondersTestRunner.getenv("WORLD_WONDERS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if WorldWondersTestRunner._test_control is not None:
            return WorldWondersTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WorldWondersTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WorldWondersTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WorldWondersTestRunner.load_test_control()
...
def load_env_local():
    WorldWondersTestRunner.load_env_local()


def env_override(m):
    return WorldWondersTestRunner.env_override(m)


def entity_data(v):
    return WorldWondersTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WorldWondersTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WorldWondersTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WorldWondersTestRunner.load_test_control()


def live_client_options():
    return WorldWondersTestRunner.live_client_options()


def live_delay_ms():
    return WorldWondersTestRunner.live_delay_ms()
